...
-}
module ParserGenerator.ParserRequirements (
-- * ParseState
ParseState (..),
showPos,
parseState,
...
tokenWithState :: Parser (TokenType a) -> Parser (Token a)
tokenWithState p = do
    ps <- getState
    t <- p
    return $ Token ps t
-- | Throws away @TokenLineComment@ and @TokenBlockComment@ based on the @Scanner@ configuration
asTokenComment :: Scanner a -> (Token a) -> [Token a]
...
asToken :: Scanner a -> Parser (TokenType a) -> Parser [Token a]
asToken conf p = do
...
operator :: Scanner a -> Parser (TokenType a)
...
keyword :: Scanner a -> Parser (TokenType a)
...
getCharParser :: Char -> (TokenType a) -> Parser (TokenType a)
...
identifierToken :: Parser (TokenType a)
identifierToken = do
    id <- ident <|> upperIdent
    return $ TokenIdentifier id
-- | Parsers for all of the 6 brackets: @[] {} ()@
brackets :: Parser (TokenType a)
...
getIdentifierTokenParser :: Bool -> Parser (TokenType a)
...
commentParser :: String -> String -> (String -> (TokenType a)) -> Parser (TokenType a)
...
blockCommentParser :: Scanner a -> Parser (TokenType a)
...
lineCommentParser :: Scanner a -> Parser (TokenType a)
...
scanParser :: Scanner a -> Parser [Token a]
...
scan :: Scanner a -> String -> Result [Token a]
scan conf str = do
...
data LanguageDefs = TokenDirective String | TokenCodeBlock String | TokenIdentifierPrime String deriving Show
...
innerCodeBlock = (codeBlock' True) <|> return ""
...
codeBlock' :: Bool -> Parser String
codeBlock' includeBraces = do
    char '{'
    str <- fmap concat $ many codeBlockAux
    postStr <- nonCurlyStr
    char '}'
    return $ withBraces includeBraces $ str ++ postStr
-- | Haskell code block, defined as follows: @{ haskellCode }@
codeBlock :: Parser (TokenType LanguageDefs)
codeBlock = fmap (TokenCustom . TokenCodeBlock) $ codeBlock' False
-- | Parser for directives of the form @%mydirective@
directive :: Parser (TokenType LanguageDefs)
directive = do
    char '%'
    str <- ident
    return $ TokenCustom $ TokenDirective str
-- | Haskell identifiers, defined as lower identifiers followed by zero or more @'@s
identifierPrime :: Parser (TokenType LanguageDefs)
identifierPrime = do
    name <- ident
    primes <- some (char '\'')
    return $ TokenCustom $ TokenIdentifierPrime (name ++ primes)
-- | All three above parsers combined into one parser
languageDefsParser :: Parser (TokenType LanguageDefs)
